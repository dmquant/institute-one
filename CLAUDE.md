...
| `app/institute/memory.py` | Analyst standing memory: versioned compacts (23:30 SGT job); `prompt_with_memory()` centralizes injection for dailies/whiteboard/mailbox/workflow steps plus analyst-scoped task/session/MCP calls. Multi-agent fan-out still builds the persona sandwich directly. |
...
| `app/institute/vectors.py` | Optional Ollama `bge-m3` + sqlite-vec substrate for archive search and similarity gates; failures degrade to FTS5 / no-vector paths without escaping to callers. |
| `app/institute/theses.py`, `app/institute/market_thesis_import.py` | Versioned thesis/lane registry plus transactional external-bundle importer; `theses` rows are the live projection and `thesis_versions` preserves revisions. |
...
| `app/institute/operator.py` | Operator loop: action feeds + SHADOW-mode router (logged suggestions only — the human gate stays human; approve re-checks the live confidence floor at consume time). Human-approved dispositions can become active recipes that short-circuit matching routes without bypassing shadow mode or approval. |
...
| `app/institute/multi_agent.py` | Fan-out/join primitives and `/api/multi-agent/run`: up to five named analysts, bounded request wait, four deterministic join modes; timed-out tasks keep running and remain queryable. |
| `app/institute/roadmap.py` | Durable roadmap backend: cards/checklists/dependencies/evidence/sessions/decisions in SQLite; `backlog.json` is seed/import/export, not the live state. |
...
11. **Bounded failure retries** — any tick that re-selects a row on failure MUST cap the retries, or a single poison row burns model quota / starves the queue forever. Two templates: (a) **attempts bound** — add an `attempts` column, atomically increment on failure, and drive the row to a terminal state (`failed`/`skipped`) via a conditional claim once `attempts >= N` (`fact_cards.attempts`, `chain_property_staging.attempts`; selection ordering must not let an un-terminal poison row keep winning). (b) **lease ownership** — a claim writes a random `lease_id`; every terminal write carries `AND lease_id = ?`; a stale-lease sweep reclaims dead owners so a slow worker's late write cannot clobber a fresh claim (`fact_cards`, `fact_extract_queue`, `factcheck_dispute_outbox`). Never leave a failure path that loops without an attempts or lease upper bound.
