...
当前执行进度（状态取自 `backlog.json`，2026-07-03——16 张种子卡片中 5 张完成 · 3 张评审中 · 2 张就绪 · 6 张待定）：

```mermaid
flowchart LR
    M0["M0 ☑ 研究手策略<br/>codex+agy 轮转"]
    M1["M1 ◔ 论点注册表<br/>2/4 完成 · 领域 API 评审中"]
    M2["M2 ◔ 证券主档与股票映射<br/>表结构评审中"]
    M3["M3 ☐ 论点感知研究队列"]
    M4["M4 ☐ 行情数据与 PIT 存储"]
    M5["M5 ☐ 预测台账"]
    M6["M6 ☐ Alpha 与纸面账本"]
    M7["M7 ◔ 控制平面<br/>API ✅ · 看板已接通（评审中）"]
    M0 --> M1 --> M2
    M1 & M2 --> M3
    M2 --> M4 --> M5 --> M6
    M1 --> M5
