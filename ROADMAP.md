...
- ☑ **P2 · No optional auth while `INSTITUTE_HOST` is settable** (S–M). `INSTITUTE_TOKEN` bearer middleware shipped (pure-ASGI, enforced when set, /health exempt, non-loopback warning; `start.sh` honors `$INSTITUTE_HOST`) — SPA/plugin/MCP clients have no token configuration surface yet. **[R1 SPA/plugin token configuration shipped in R1 (localStorage + plugin setting); MCP rides the same ASGI middleware.]**
...
- ☑ **P3 · Test gaps** (M–L): no API-route tests, no MCP round-trip test, no vault-exporter handler tests, no scheduler gating test → tracked in Phase 8 (those four suites now exist; frontend SSE automation shipped — vitest+jsdom useSSE/askStream suite, 16 passed; removable skips cleared 2026-07-20: S4-P0-02/M8-002 had already killed the over-broad markers and the D4 probe, and the skip-cleanup pass then ported the last 2 real-bundle importer tests onto the synthetic subset fixture and deleted the dead A2/partition skip guards — every surviving skip is a deliberate opt-in gate; see Phase 8 test coverage and docs/history/PATCH-NOTES-SKIP-CLEANUP.md).
...
- ☑ **Whiteboard similarity gate + diversity pick** (M). Before kickoff: cosine vs recent boards — ≥0.85/14d skip, ≥0.65/30d augment the prompt with "BUILD ON prior work"; topic pick gets a diversity penalty instead of pure max-score. Gate logic and configurable defaults have shipped. **[Real bge-m3 calibration executed 2026-07-22: `INSTITUTE_CALIBRATION_REAL=1` against local Ollama bge-m3 — the 50+ known-pair corpus, tier-separation and classifier-matrix assertions all passed (7 passed, incl. `test_real_bge_m3_calibration`). Formal M8-004 card acceptance remains with the operator.]**
...
- ☑ **Executor depth** (M). Retry lineage persisted (`fallback_chain`/`lineage_root`, migration 0024) with a partial unique index as the cross-process in-flight dedup key; `rate-limit-revival` scheduler job resurrects cooled-down `rate_limited` tasks (≤3/tick, one-shot claim marker). Remaining: per-hand queue-depth cap with `overcommitted` fast-fail. **[R1 Per-hand queue-depth cap with overcommitted fast-fail shipped in R1 (migration 0028, INSTITUTE_HAND_QUEUE_DEPTH).]**
- ☑ **Prompt-overrides** (M). `prompt_overrides` table (shadow → active → retired, per scope) layered over the prompt constants, with an operations API — makes prompt iteration data instead of code (and relaxes CLAUDE.md rule 4 safely). **[R1 Shipped in R1: migration 0029, shadow→active→retired lifecycle, resolve() over prompt constants (byte-stable defaults), /api/prompt-overrides.]**
...
- ☑ **Vault projection** (M). `Chain/<entity>.md` note per node (managed regions); **`## Entities` wikilink footers** injected into every exported note; Dataview inline typed relations (`supplier_of:: [[台积电]]`); `_meta/Dashboards.md` starter Dataview queries. (Entity notes, wikilink footers + historical footer backfill shipped. **[Closed 2026-07-20: typed relations render as `relation:: [[slug]]` inline fields on entity notes — identity role→field mapping over the snake_case vocabulary, pinned by tests; `## Entities` footers extended to the remaining exporter kinds (factcheck digest, paper-book journal, research tree, bilingual twins). Known follow-up closed 2026-07-22: `REPROJECT_KINDS` now covers the new kinds (factcheck / paper-book-journal / research_tree / committee) — see docs/history/PATCH-NOTES-VAULT-PROJECTION.md for the original gap.]**)
- ☑ **Properties + conflicts** (L, optional). `chain_properties` with the hybrid supersede/conflict policy; conflicts surface as operator actions (Phase 6). — skipped this build: optional. **[R1 Shipped in R1: migration 0030, hybrid supersede/conflict with operator adjudication, transactional with the conflict action.]**
...
- ☑ **Portfolios L1–L3 + Sunday proposer** (L, optional). Per-analyst virtual portfolios; Sun 22:00 proposer. — skipped this build: optional (no portfolio domain model yet). **[R1 Shipped in R1: migration 0032, conviction-tiered L1/L2/L3 per analyst, Sunday 22:00 proposer job, conditional-claim adjudication.]**
...
- ☑ **Recipes / observations / proposals / effect measurement** (L). Minimal recipe loop shipped: an approved disposition can be promoted to a recipe, recurring same-kind actions get zero-model-call suggestions (still shadow, still through the human approve gate); observations/proposals/parameter-history/effect-measurement remain open (M8-008). **[R1 Full self-improvement loop shipped in R1 (M8-008): migration 0026, observations→proposals→human approve gate→effect measurement→parameter history, 00:15 daily job.]**
...
- ☑ **Committee** (M). `workflows/committee.json` deliberation (mine recent whiteboard summaries for the week's biggest disagreement; 3 analysts argue; editor compiles a verdict with dissent recorded); 22:00 SGT on committee days; idempotent advance; vault `Committee/`. **[R1 M8-012 shipped the missing persistence: multi_agent_groups/multi_agent_runs (migration 0027) with input snapshots, partial-spawn recovery via parent_run_id, and the Committee/<date> vault projection — 49 tests.]**
- ☑ **Research projects** (M). Group research runs + boards + threads under a named long-running project; project page; project digest endpoint. (Backend container + link/enqueue rails, idempotent unlink, archive/unarchive with a 409 link guard, and structured + markdown digest endpoints shipped; remaining: SPA project page, MCP `project_id` passthrough.) **[R1 M8-010 closed in R1: MCP project_id passthrough was the only real gap; SPA page and ops API pre-existed.]**
...
- ☑ **Test coverage push** (L). TestClient suites per router, MCP JSON-RPC round-trip, vault-exporter handler tests (synthetic bus events → assert notes), scheduler gating test, restart-recovery tests. (All five named suites shipped; vitest+jsdom frontend suite now covers the useSSE state machine — bootstrap paging, cursor reconnect, ring eviction, watchdog, filtered-feed reconcile — plus askStream NDJSON parsing; removable skips cleared 2026-07-20 — S4-P0-02 split the over-broad bundle marker, M8-002 replaced the D4 restart probe with a deterministic domain-function fixture, and the skip-cleanup pass ported the last 2 real-bundle importer tests onto the synthetic subset fixture (tests never read the commercial dataset) and deleted the dead A2/partition skip guards in the MCP suites. The default run keeps exactly 1 deliberate skip — the INSTITUTE_NET_TESTS=1 network smoke — plus opt-in gates that don't fire here: INSTITUTE_CALIBRATION_REAL, sqlite-vec, plutil. See docs/history/PATCH-NOTES-SKIP-CLEANUP.md.)
